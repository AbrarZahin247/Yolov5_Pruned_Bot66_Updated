...
from prune_net import direct_prune
...
def train(hyp, opt, device, callbacks):
    ...
    compute_loss = ComputeLoss(model,add_gamma_loss=True,sr=opt.sr)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        if(epoch==opt.pruning_epoch):
            model=direct_prune(model=model,cfg=cfg,device=device,save_dir="pruned_models",pruning_ratio=opt.pruning_ratio)
            compute_loss=ComputeLoss(model,add_gamma_loss=False,sr=opt.sr)()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--pruning_epoch', type=int, default=2, help='train model with l1 sparsity')
    parser.add_argument('--pruning_ratio', type=float, default=0.5,help='sparsity rate')
